/* ==========================================================
   RED — ESTRUCTURA FUNCIONAL Y DEL CUIDADO (EFC) — POT Bogotá
   ...
   - Conexiones tomadas 1 a 1 de la tabla EFC (páginas 30, 43, 122,
     125, 126 y 239–241). Todas con flecha ➡️, línea continua.
   - Clic en una línea -> panel con Relación / Tipo / Texto exacto / Página.
   ...
   - Todas las relaciones son "Soporte": línea naranja continua.
   ========================================================== */

const SVG_NS = "http://www.w3.org/2000/svg";
const XHTML_NS = "http://www.w3.org/1999/xhtml";

/* -------- Nodos: conceptos EFC con posición fija -------- */
const ODS_NODES = [
  { id: "redvial",    num: "", name: "RED VIAL",                              icon: "fa-road",            color: "#2fd4c8", x: 120,  y: 240, r: 58 },
  { id: "corredores", num: "", name: "CORREDORES\nVERDES",                    icon: "fa-tree",            color: "#4ade80", x: 380,  y: 100, r: 60 },
  { id: "transporte", num: "", name: "TRANSPORTE\nPÚBLICO",                   icon: "fa-bus",             color: "#ef9552", x: 800,  y: 160, r: 64 },
  { id: "ciclorutas", num: "", name: "CICLORRUTAS",                           icon: "fa-person-biking",   color: "#a276f2", x: 620,  y: 380, r: 58 },
  { id: "equip",      num: "", name: "EQUIPAMIENTOS",                         icon: "fa-school",          color: "#5b8def", x: 1120, y: 380, r: 60 },
  { id: "vivienda",   num: "", name: "VIVIENDA",                              icon: "fa-house",           color: "#f5c945", x: 1380, y: 170, r: 58 },
  { id: "manzanas",   num: "", name: "MANZANAS\nDEL CUIDADO",                 icon: "fa-people-roof",     color: "#f76fb0", x: 1010, y: 600, r: 60 },
  { id: "sserv",      num: "", name: "SERVICIOS\nSOCIALES",                   icon: "fa-hand-holding-heart", color: "#8de8c4", x: 720,  y: 620, r: 54 },
  { id: "scuidado",   num: "", name: "SERVICIOS\nDE CUIDADO",                 icon: "fa-heart-pulse",     color: "#ff7eb6", x: 1280, y: 600, r: 56 },
];
...
/* Tipos de relación de la tabla EFC: soporte (naranja) */
const TYPE_STYLE = {
  soporte: { color: "#ef9552", width: 2.6, label: "Soporte" },
};

/* -------- Aristas: tabla EFC del POT, 1 a 1 (flecha ➡️, continua) -------- */
const RAW_EDGES = [
  { s: "redvial",    t: "transporte", type: "soporte", directa: true, pagina: 43,   sustento: "“Además del Metro, Bogotá necesita con urgencia ampliar sus entradas y salidas, tapar más huecos, hacer más vías, ciclorrutas, cables y corredores verdes con buses eléctricos para que el transporte público de calidad llegue a todas partes, conecte a la gente, la saque del trancón y la contaminación.”" },
  { s: "corredores", t: "transporte", type: "soporte", directa: true, pagina: 30,   sustento: "“Y que, en todo caso, las diversas zonas de la ciudad estén conectadas por un sistema multimodal de transporte público, colectivo, de energías limpias y renovables basadas en la red Metro y alimentadas por los demás modos y medios de transporte público como los corredores verdes, los cables y las ciclorrutas.”" },
  { s: "corredores", t: "ciclorutas", type: "soporte", directa: true, pagina: 239,  sustento: "“Por eso, además del Metro, y para alimentarlo y complementarlo, están los corredores verdes, con diseño ecosistémico, transporte público eléctrico, ciclorrutas seguras y andenes, plazas y espacios de encuentro...”", paginaTexto: "p. 239–241" },
  { s: "equip",      t: "vivienda",  type: "soporte", directa: true, pagina: 126,  sustento: "“Por un lado, priorizamos que los colegios o equipamientos educativos estén cerca de la vivienda o incluso cerca del trabajo de los padres.”" },
  { s: "equip",      t: "manzanas",  type: "soporte", directa: true, pagina: 125,  sustento: "“Aprovechar los equipamientos existentes como anclas de las Manzanas del Cuidado, para que en estos diferentes entidades del Distrito cuiden a quienes nos cuidan, fue el cuello de botella que se resolvió con el pot.”" },
  { s: "manzanas",   t: "sserv",     type: "soporte", directa: true, pagina: 126,  sustento: "“...cualifica los servicios sociales del Distrito y hace efectiva la articulación interinstitucional.”" },
  { s: "manzanas",   t: "scuidado",  type: "soporte", directa: true, pagina: 122,  sustento: "“Las Manzanas del Cuidado son áreas acotadas que agrupan diversas infraestructuras para brindar servicios de manera simultánea y articulada a las personas cuidadoras, a quienes ellas cuidan y a sus familias.”" },
];
...
function showEdgeInfo(index) {
  ...
  document.getElementById("edgeInfoPage").textContent =
    edge.paginaTexto ? `Página POT: ${edge.paginaTexto}` : (edge.pagina != null ? `Página POT: p. ${edge.pagina}` : "Página: por confirmar");
...
const NODE_INSIGHTS = {
  conceptos:    ["transporte", "manzanas"],
  conexiones:   ["corredores", "equip", "manzanas"],
  tipos:        [],
};
...
function toggleInsight(key) {
  ...
  if (key === "soporte") {
    setSpotlightTypes(["soporte"]);
  } else if (key === "tipos") {
    /* mostrar el único tipo de relación de la red: soporte (naranja) */
    setSpotlightTypes(["soporte"]);
  } else if (key === "conexiones") {
    const ids = NODE_INSIGHTS.conexiones.flatMap(id => [id, ...RAW_EDGES.filter(e => e.s === id || e.t === id).flatMap(e => [e.s, e.t])]);
